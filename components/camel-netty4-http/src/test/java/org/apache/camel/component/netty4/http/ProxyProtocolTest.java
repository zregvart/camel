...
public class ProxyProtocolTest {
    ...
    public ProxyProtocolTest(final Function<RouteBuilder, RouteDefinition> variant, final String url) throws Exception {
        ...
        context.addRoutes(new RouteBuilder() {
            ...
            public void configure() throws Exception {
                ...
                // http://localhost:originPort/path
                from("netty4-http:http://localhost:" + ORIGIN_PORT + "/path")
    ...
    public void shouldProvideProxyProtocolSupport() {
        final NettyHttpEndpoint endpoint = context.getEndpoint("netty4-http:proxy://localhost", NettyHttpEndpoint.class);
    ...
    public static Iterable<Object[]> routeOptions() {
        final Function<RouteBuilder, RouteDefinition> single = r -> r.from("netty4-http:proxy://localhost:" + PROXY_PORT)
            .process(ProxyProtocolTest::uppercase)
            .to("netty4-http:http://localhost:" + ORIGIN_PORT)
            .process(ProxyProtocolTest::uppercase);

        final Function<RouteBuilder, RouteDefinition> dynamicPath = r -> r.from("netty4-http:proxy://localhost:" + PROXY_PORT)
            .process(ProxyProtocolTest::uppercase)
            .toD("netty4-http:http://localhost:" + ORIGIN_PORT + "/${headers." + Exchange.HTTP_PATH + "}")
            .process(ProxyProtocolTest::uppercase);

        final Function<RouteBuilder, RouteDefinition> dynamicUrl = r -> r.from("netty4-http:proxy://localhost:" + PROXY_PORT)
            .process(ProxyProtocolTest::uppercase)
            .toD("netty4-http:"
    ...
    private static void origin(final Exchange exchange) {
        ...
        if ("text/plain".equals(message.getHeader(Exchange.CONTENT_TYPE))) {
            ...
            assertThat(body).isEqualTo(body.toUpperCase(Locale.US));
        }
